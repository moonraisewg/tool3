...
export type FormBuySol = z.infer<typeof formSchema>;

const SOL_MINT = "So11111111111111111111111111111111111111112";

export default function BuySollMainnet() {
    ...
    const fetchPrice = useCallback(
        async (inputAmount: string) => {
            try {
                ...
                const tokenMint = selectedToken?.address || "";
                const response = await fetch(`https://lite-api.jup.ag/price/v2?ids=${tokenMint}&vsToken=${SOL_MINT}`);

                const result = await response.json();
                const priceInSOL = result?.data[tokenMint]?.price;

                if (priceInSOL) {
                    const amountValue = parseFloat(inputAmount);
                    if (!isNaN(amountValue) && amountValue > 0) {
                        if (isSwapped) {
                            const tokenAmount = amountValue / priceInSOL;
                            form.setValue("amount", tokenAmount.toFixed(selectedToken?.decimals || 6));
                        } else {
                            const solAmount = amountValue * priceInSOL;
    ...
    const onSubmit = async (values: FormBuySol) => {
        try {
            setLoading(true);
            ...
            const response = await fetch("/api/buy-sol", {
                method: "POST",
                headers: {
            ...
            const confirmResponse = await fetch("/api/buy-soll/confirm", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
            ...
            toast.success("🎉 Buy sol mainnet Successful!", {
                description: `You sold ${values.amount} ${selectedToken.symbol} for ${values.solAmount} SOL Mainnet`,
                action: {
    ...
    return (
        <div className={`md:p-2 max-w-[550px] mx-auto my-2 ${!isMobile && "border-gear"}`}>
            <h2 className="text-2xl font-bold text-gray-900 mb-8 text-center">Buy SOL Mainnet</h2>
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 flex flex-col justify-center">
                    ...
                    <Button
                        ...
                        disabled={loading}
                    >
                        {loading ? "Processing..." : "Buy SOL Mainnet"}
                    </Button>
                </form>
            </Form>
        </div>
