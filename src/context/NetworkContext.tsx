...
export function NetworkProvider({ children }: { children: ReactNode }) {
    ...
    useEffect(() => {
        ...
        const isApiPath = route.api && route.api.some(apiPath => {
            const currentPathWithoutQuery = currentPath.split('?')[0];
            return currentPathWithoutQuery === apiPath || currentPathWithoutQuery.startsWith(apiPath + '/');
        });
        if (isApiPath) {
        ...
        if (newNetwork !== network) {
            ...
            } else if (newNetwork === WalletAdapterNetwork.Mainnet && !validMainnetPaths.includes(currentPath)) {
                if (route.mainnet.length > 0 && route.mainnet[0].url) {
            ...
            } else if (network === WalletAdapterNetwork.Mainnet && !validMainnetPaths.includes(currentPath)) {
                if (route.mainnet.length > 0 && route.mainnet[0].url) {
                    router.push(route.mainnet[0].url);
                }
            }
